...
DigestAdapter *digest_adapter_copy_with_alloc(const DigestAdapter *da)
{
    ...
    if (ctx->mode_id == DIGEST_MODE_GOST34311) {
        ...
        CHECK_NOT_NULL(params->mode.dstu7564 = dstu7564_copy_with_alloc(ctx->mode.dstu7564));
    } else {
        SET_ERROR(RET_PKIX_UNSUPPORTED_DIGEST_ALG);
    }
...
cleanup:
    ...
    prng_free(prng);
    sha2_free(sha2_ctx);
    /* Upstream не звільняв sha1_ctx: витік на кожній операції ПІДПИСУ шляхом
...
cleanup:
    ...
    dstu7564_free(kupyna_ctx);
    sha2_free(sha2_ctx);
    /* Той самий витік у шляху ПЕРЕВІРКИ підпису. */
    sha1_free(sha1_ctx);
